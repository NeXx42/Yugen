...
export default function () {
    ...
    useEffect(() => {
        ...
        api.notification_Count().then(() => setAuthenticated(true)).catch(() => setAuthenticated(false));
        ...
        const callback = () => {
            if (searchQuery.length >= 3) {
                api.catalog_Search({
                    ...
                    setQuickSearchItems(() => r.data.sort((a, b) => (a.title ?? "").localeCompare(b.title ?? "")))
                });
            }
        };

        let searchEvent = setTimeout(callback, 100);
    ...
    return (<div className="Topbar">
        ...
        <div className="Topbar_Centre">
            <div className="Topbar_SearchInput">
...

            {
                quickSearchOpen && searchQuery?.length >= 3 && (quickSearchItems?.length ?? 0) > 0 && (
                    <div ref={searchRef} className="Topbar_QuickSearch">
                        {
        ...
        <div className="Topbar_Right">
            {
                authenticated && (<>
                    <button className="Topbar_Right_Notifications Topbar_btn" onClick={() => toggleFoldout("Notifications")}>
                        ...
                        {notificationCount > 0 && (
                            <a>{Math.min(notificationCount, 99)}</a>
                        )}
                    </button>
                </>)
            }
            <button className="Topbar_Right_Profile Topbar_btn" onClick={() => setIsProfileOpen(true)}>
